...
class TestRestInterface extends AnyWordSpecLike with Matchers with ScalatestRouteTest with DefaultJsonProtocol {
  ...
  "The Api` service" should {
    "return a '{\"name\":\"kruiden\",\"sequence\":40}' response for GET requests to /api/category/kruiden" in {
      Get("/api/category/kruiden") ~> apiRoute ~> check
      { status should be(OK)
    ...
    "return a recipe response for GET requests to /api/recipe/Lasagne%20met%20gehakt" in {
      Get("/api/recipe/Lasagne%20met%20gehakt") ~> apiRoute ~> check
      { status should be(OK)
