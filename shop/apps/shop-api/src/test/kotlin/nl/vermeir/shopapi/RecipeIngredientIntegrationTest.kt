...
class RecipeIngredientIntegrationTest {
  ...
  fun `a RecipeIngredient should be updated`() {
    val ri1 = createRecipeIngredient()
    val ri1saved = save(ri1, path = "recipe-ingredient")
    val ri2saved = save(ri1, path = "recipe-ingredient")
    val ri1savedAgain = save(ri1saved.copy(ingredientId = ri2saved.ingredientId), path = "recipe-ingredient")
    ri1savedAgain.ingredientId shouldBe ri2saved.ingredientId
  }
  ...
  fun `a RecipeIngredient should be returned by findById`() {
    val ri1 = createRecipeIngredient()
    val ri1saved = save(ri1, path = "recipe-ingredient")
    val ri1ById: RecipeIngredient = load("recipe-ingredient/${ri1saved.id}", listOf())
